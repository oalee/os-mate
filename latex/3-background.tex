...
% Ivy is a unified templated deep learning framework that abstracts existing deep learning frameworks and unifies their core functions to exhibit consistent call signatures, syntax, and input-output behavior. Ivy provides a unified class interface for neural networks and implements new high-level framework-agnostic functions and classes that can be used alongside framework-specific code. Ivy supports TensorFlow, PyTorch, MXNet, Jax, and NumPy. Ivy also provides pure-Ivy libraries for mechanics, 3D vision, robotics, and differentiable environments.

% While Ivy and Yerbamaté share some similarities in their efforts to abstract and unify existing deep learning frameworks, they differ in their approaches. Ivy emphasizes providing a unified class interface for neural networks and implementing new high-level framework-agnostic functions, while Yerbamaté focuses on providing a modular architecture that separates concerns and allows for independent development and testing of data, models, trainers, and experiments. In this sense, Yerbamaté is more agnostic to the underlying framework, while Ivy is more concerned with unifying the existing frameworks.
